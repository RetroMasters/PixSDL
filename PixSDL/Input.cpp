...
namespace pix
{
	...
	int GamepadInput::AddAllGamepads()
	{
		...
		if (gamepadCount > 100) gamepadCount = 100; // Just a sanity measure to prevent huge numbers
	...
	void GamepadInput::AddGamepad(int deviceIndex)
	{
		...
		for (const Gamepad& gamepad : gamepads_)
			if (gamepad.IsInitialized && gamepad.JoystickID == joystickID)
				return;
		...
		for (Gamepad& gamepad : gamepads_)
		{
			if (!gamepad.IsInitialized)
			{
				gamepad.Set(gameController, rumbleHandle, joystickID, true);
				return;
			}
		}
	...
	int GamepadInput::RemoveAllGamepads()
	{
		...
		for (const Gamepad& gamepad : gamepads_)
		{
			if (gamepad.IsInitialized)
			{
				RemoveGamepad(gamepad.JoystickID);
				removedCount++;
			}
		}
	...
	void GamepadInput::RemoveGamepad(SDL_JoystickID joystickID)
	{
		for (Gamepad& gamepad : gamepads_)
		{
			if (gamepad.IsInitialized && gamepad.JoystickID == joystickID)
			{
				if (gamepad.RumbleHandle)
				{
					SDL_HapticRumbleStop(gamepad.RumbleHandle);
					SDL_HapticClose(gamepad.RumbleHandle);
				}
				...
				gamepad.Set(nullptr, nullptr, -1, false);
	...
	int GamepadInput::GetActiveGamepadCount() const
	{
		...
		for (const Gamepad& gamepad : gamepads_)
			if (gamepad.IsInitialized) count++;
	...
	bool GamepadInput::StartRumble(int gamepadIndex, float force, int duration)
	{
		if (!IsValidGamepadIndex(gamepadIndex) || force <= 0.0f || duration == 0) return false;
	...
	bool GamepadInput::StopRumble(int gamepadIndex)
	{
		if (!IsValidGamepadIndex(gamepadIndex)) return false;
