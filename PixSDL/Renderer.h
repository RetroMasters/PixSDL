...
namespace pix
{
	// The Renderer singleton manages the lifetime of the SDL_Renderer and exposes core rendering functionality.
	// 
	// Must be initialized with Init() before normal use.
	// 
	// Philosophy:
	// The purpose of the Renderer singleton is to centralize core rendering functionality.
	// SDL types are used where possible to preserve zero-overhead usage and direct
	// compatibility with SDL documentation. Engine-specific types, such as textures,
	// are used where correctness and safe usage should be guaranteed by the API.
	// 
	// Rendering is a critical subsystem; after Init() failure, normal engine execution should not continue.
	// Renderer methods therefore assume successful initialization and do not perform repeated initialization checks.
