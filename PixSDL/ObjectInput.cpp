...
namespace pix
{
	...
	{
		for (VirtualAxis& axis : virtualAxes_)
			axis.ClearState();
	}

	void ObjectInput::ResetVirtualSourceState()
	{
		for (VirtualInputPump& pump : virtualInputPumps_)
			pump.SetSourceState(0.0f);
	}
	...
	{
		for (VirtualAxis& axis : virtualAxes_)
			axis.SetAxisState(0.0f);
	}

	void ObjectInput::PumpInput()
	{

		for (KeyboardInputPump& pump : keyboardInputPumps_)
			pump.Pump();

		for (MouseButtonInputPump& pump : mouseInputPumps_)
			pump.Pump();

		for (GamepadInputPump& pump : gamepadInputPumps_)
			pump.Pump();
	...
	{
		for (KeyboardInputPump& pump : keyboardInputPumps_)
			pump.SetVirtualAxis(virtualAxes_[pump.GetCachedAxisID()]);
	...
	{
		for (const VirtualAxis& axis : virtualAxes_)
			if (axis.GetName() == axisName) return &axis;
