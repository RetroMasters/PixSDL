...
namespace pix
{
	...
	public:
		...
		friend Rotation2D GetInterpolated(const Rotation2D& startRotation, const Rotation2D& endRotation, float interpolationAlpha);
		...
		// Checks for exact equality of the stored rotation axis
		bool operator==(const Rotation2D& other) const;

		// Checks for exact inequality of the stored rotation axis
		bool operator!=(const Rotation2D& other) const;
	...
	public:
		...
		// Checks for exact equality of the stored rotation axes
		bool operator==(const Rotation3D& other) const;

		// Checks for exact inequality of the stored rotation axes
		bool operator!=(const Rotation3D& other) const;


	private:

		Rotation3D(const Vec3f& xAxis, const Vec3f& yAxis); // Used for AddGlobalRotation(Vec3f rotAxis, float degrees)
	...
	// Returns a linearly interpolated Rotation2D by interpolating between the rotation axes using interpolationAlpha.
	// Returns startRotation if interpolationAlpha <= 0.0f and endRotation if interpolationAlpha >= 1.0f.
	...
	// Returns a linearly interpolated Rotation3D by interpolating between the rotation axes using interpolationAlpha.
	// Returns startRotation if interpolationAlpha <= 0.0f and endRotation if interpolationAlpha >= 1.0f.
	...
	struct Transform2D
	{
		...
		explicit Transform2D(const Vec2& position, const Vec2f& scale = Vec2f(1.0f, 1.0f), const Rotation2D& rotation = Rotation2D());

		// Applies the transform to an array of points.
		...
		// Applies the inverse transform to an array of points.
		// points must be non-null when count > 0. Caller must ensure count does not exceed the array length.
		void InverseTransformPoints(Vec2* points, int count) const;

		void InverseTransformPoint(Vec2& point) const;

		// Checks for exact equality of the transform components
		bool operator==(const Transform2D& other) const;

		// Checks for exact inequality of the transform components
		bool operator!=(const Transform2D& other) const;
	...
	struct Transform3D
	{
		...
		explicit Transform3D(const Vec3& position, const Vec3f& scale = Vec3f(1.0f, 1.0f, 1.0f), const Rotation3D& rotation = Rotation3D());
	...
	// Returns a linearly interpolated Transform2D by interpolating the position, scale, and rotation between startTransform and endTransform using interpolationAlpha.
	// Returns startTransform if interpolationAlpha <= 0.0f and endTransform if interpolationAlpha >= 1.0f.
	// Otherwise, it is functionally equivalent to calling the linear interpolation functions manually on the transform members.
	Transform2D GetInterpolated(const Transform2D& startTransform, const Transform2D& endTransform, float interpolationAlpha);

	// Returns a linearly interpolated Transform3D by interpolating the position, scale, and rotation between startTransform and endTransform using interpolationAlpha.
	// Returns startTransform if interpolationAlpha <= 0.0f and endTransform if interpolationAlpha >= 1.0f.
	// Otherwise, it is functionally equivalent to calling the linear interpolation functions manually on the transform members.
