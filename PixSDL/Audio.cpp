...
namespace pix
{
	...
	bool Audio::ReloadMusic(const std::string& path)
	{
	...
	{
		if (!isInitialized_ || !musicTrack_) return false;
	...
	void Audio::SetMusicRepeatCount(int repeatCount)
	{
		if (repeatCount < -1) repeatCount = -1;

		musicRepeatCount_ = repeatCount;
	}

	bool Audio::SetMusicPosition(double timePosition)
	{
		if (!isInitialized_ || !musicTrack_) return false;
	...
	bool Audio::PlaySoundChunk(int channel, Mix_Chunk* soundChunk, int repeatCount)
	{
		if (!isInitialized_) return false;
	...
	bool Audio::StopSoundChunk(Mix_Chunk* soundChunk)
	{
		if (!isInitialized_) return false;
