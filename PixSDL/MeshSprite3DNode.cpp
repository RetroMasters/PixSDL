...
namespace pix
{
	...
	MeshSprite3DNode::MeshSprite3DNode(const Mesh2D* mesh, const Transform3D& transform) : MovableObject3D(transform),
		Mesh(mesh)
	{}

	MeshSprite3DNode::MeshSprite3DNode(const Mesh2D* mesh, const Transform3D& transform, const Transform3D& prevTransform) : MovableObject3D(transform, prevTransform),
		Mesh(mesh)
	{}

	MeshSprite3DNode::MeshSprite3DNode(const Mesh2D* mesh, const Vec3& position, const Vec3f& scale, const Rotation3D& rotation) : MovableObject3D(position, scale, rotation),
		Mesh(mesh)
	{}

	MeshSprite3DNode::~MeshSprite3DNode()
	{
	...
	bool MeshSprite3DNode::SetParent(MeshSprite3DNode* newParent)
	{
		if (newParent == parent_) return true;

		MeshSprite3DNode* currentParent = newParent;
	...
	MeshSprite3DNode* MeshSprite3DNode::GetParent() const
	{
		return parent_;
	}

	const std::vector<MeshSprite3DNode*>& MeshSprite3DNode::GetChildren() const
	{
		return children_;
	}

	Transform3D MeshSprite3DNode::GetGlobalTransform() const
	{
		const MeshSprite3DNode* parent = parent_;
	...
	Transform3D MeshSprite3DNode::GetPrevGlobalTransform() const
	{
		const MeshSprite3DNode* parent = parent_;
