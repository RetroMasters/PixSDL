...
// Generic 3D geometry utilities for raw points and point arrays.
// These functions operate on vector-like types with X, Y, and Z members, such as Vec3, Vec3f, and Vec3i.
...
namespace pix
{
	...
	template<typename T> inline T GetBoundsSize3D(const T* points, int count)
	{
		T min = points[0];
		T max = min;
	...
	// points must point to the first element of an array with count elements.
	// Inputs are not validated; points must not be null, and count must be greater than 0.
	template<typename T> inline T GetMinBounds3D(const T* points, int count)
	{
		T min = points[0];
	...
	// points must point to the first element of an array with count elements.
	// Inputs are not validated; points must not be null, and count must be greater than 0.
	template<typename T> inline T GetMaxBounds3D(const T* points, int count)
	{
		T max = points[0];
	...
	// minBounds and maxBounds are not validated; minBounds must be less than or equal to maxBounds on all axes.
	template<typename T> inline bool IsInsideBounds3D(const T& point, const T& minBounds, const T& maxBounds)
	{
